...
核心目标是：
将用户在 ChatGPT 项目“TB Step by Step”中已经验证有效的工作流程，迁移为 Claude Code 本地项目中的可复用工作流。
...
Claude Code 执行时应读取 `.env`，再根据 `config/model.json` 中的 `api_key_env` 获取实际密钥。
